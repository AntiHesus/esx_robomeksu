				...
				while GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), garageCoords2[1], garageCoords2[2], garageCoords2[3], true) < 50 do
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) > 5 then
						...
						if vaihe1 == true and vaihe2 == false and vaihe3 == false and vaihe4 == false and vaihe5 == false and vaihe6 == false then
							ESX.ShowNotification("~p~[Viron autokorjaamo]~w~~n~Tere! Pistetäähä auto kuntoo ~n~Oota rahus!")
							ESX.ShowNotification("~p~[Viron autokorjaamo]~w~~n~Arviolta sanoisin et menis noin..~p~ 1 min")
						end
...
RegisterNetEvent('robomeksu:onnistunut')
AddEventHandler('robomeksu:onnistunut', function(price)
	...
	korjauksessa = true
	ESX.ShowNotification("Maksoit: ~r~500€")
	Citizen.Wait(3000)
	payalert = true 
	Citizen.Wait(3000)
	payalert = false 
	...
	Citizen.Wait(5000)
	vaihe6 = false 
	ESX.ShowNotification("~p~[Viron autokorjaamo]~w~~n~No nyt on ~g~valmista!")
	mikaajoneuvo = false 
	Wait(5000)
	korjauksessa = false
end)
...
RegisterNetEvent('robomeksu:eirahaa')
AddEventHandler('robomeksu:eirahaa', function(moneyleft)
	ESX.ShowNotification("Nostaha vähän lisää rahaa, sulta puuttuu: ~r~" .. moneyleft .. " €")
end)
...
function KorjausMenu()
    ...
    local elements = {
        { label = 'Korjaus <font color="red">500€</font>', value = "korjaus" },
    }

    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'robomeksu_menu',
        {
            title    = "Viron autokorjaamo",
            align    = 'bottom-right',
            elements = elements
        },
    function(data, menu)
        ...
        if action == "korjaus" then
			TriggerServerEvent('robomeksu:rahatarkistus')
    	end
...
--blipit
local blips = {
	{title="Korjaamo", colour=42, id=446, x=1149.82, y=-777.67, z=45.565},
	{title="Korjaamo", colour=42, id=446, x=257.30, y=2598.17, z=45.565},
	{title="Korjaamo", colour=42, id=446, x=-77.96, y=6419.24, z=45.565},
}
